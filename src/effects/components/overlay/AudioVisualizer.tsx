...
const WaveformPanel: React.FC<VisualizerFrameProps & {waveform: number[]}> = ({
  colorTheme,
  accentColor,
  align,
  durationInFrames,
  ...
  const fadeOutStart = Math.max(fadeInFrames + 12, durationInFrames - fadeOutFrames);
  const opacity = interpolate(frame, [0, fadeInFrames, fadeOutStart, durationInFrames], [0, 1, 1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const translateY = interpolate(frame, [0, fadeInFrames, fadeOutStart, durationInFrames], [24, 0, 0, 28], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
...
const VisualizerWithAudioData: React.FC<VisualizerFrameProps & {audioSrc: string}> = (props) => {
  ...
  const waveform = useMemo(() => {
    if (audioData) {
      return visualizeAudio({
        ...
        frame: props.frame,
        fps: props.fps,
        numberOfSamples: props.sampleCount,
        smoothing: true,
      });
    }
    return fallbackWave(props.frame, props.sampleCount);
  }, [audioData, props.frame, props.fps, props.sampleCount]);

  return <WaveformPanel {...props} waveform={waveform} />;
};

const VisualizerWithoutAudio: React.FC<VisualizerFrameProps> = (props) => {
  const waveform = useMemo(() => fallbackWave(props.frame, props.sampleCount), [props.frame, props.sampleCount]);
  return <WaveformPanel {...props} waveform={waveform} audioSrc={undefined} />;
};
...
export const AudioVisualizer: React.FC<AudioVisualizerProps> = ({
  ...
  const baseProps: VisualizerFrameProps = {
    colorTheme,
    accentColor,
    align,
    durationInFrames,
  ...
  if (audioSrc) {
    return <VisualizerWithAudioData {...baseProps} audioSrc={audioSrc} />;
  }
  return <VisualizerWithoutAudio {...baseProps} />;
};
